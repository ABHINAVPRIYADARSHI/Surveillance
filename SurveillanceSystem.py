...
class SurveillanceSystem(object):
   ...
   def __init__(self):
        ...
        self.drawing = True 
        self.alarmState = 'Armed' # Alarm states - Disarmed, Armed, Triggered
        self.alarmTriggerd = True
   ...
   def check_camera_events(self,alert):   
        ...
        if alert.camera != 'All':  # Check cameras   
            ...
            if alert.event == 'Recognition': #Check events
                ...
                for person in self.cameras[int(alert.camera)].people.values():
                    ...
                    if alert.person == person.identity: # Has person been detected
                        ...
                        if ( person.confidence) >= alert.confidence:
            ...
            if alert.event == 'Recognition': # Check events
		logger.info( "alertTest6" + alert.camera)
                ...
                with  self.camerasLock :
                    for camera in self.cameras: # Look through all cameras
                        if camera.motion == True: # Has motion been detected
                            camNum=self.cameras.index(camera)
                            cv2.imwrite("notification/image.png", camera.processing_frame)#
                            self.take_action(alert,camNum)
                            return True
   ...
   def take_action(self,alert,camNum): 
   ...
   def send_email_notification_alert(self,alert,camNum):
      logger.info("Sending mail")
      """ Code produced in this tutorial - http://naelshiab.com/tutorial-send-email-python/"""
      fromaddr = "rush2rain@gmail.com"
      toaddr = alert.emailAddress
      tophone = "+918892674619"
      fromphone="+19493914209"
      account_sid = 'AC5cdfeafddb0e07b046f33124c2dd686b'
      auth_token = 'cb82ab838981935cfac8e5a025c1ba83'
      ...
      body = "NOTIFICATION ALERT:" +  alert.alertString + "     "+(str(camNum))
      logger.info("body")
...
class Alert(object): 
    ...
    def __init__(self,alarmState,camera, event, person, actions, emailAddress, confidence):   
        ...
        if  event == 'Motion':
            ...
            self.alertString = person + " was recognised  "+ " with a confidence greater than " + str(confidence)+ " in camera number"
        ...
        if emailAddress == None:
            self.emailAddress = "rush2rain@gmail.com"
        else:
